...
- **`bench` no longer gates on `gfortran_seconds` (issue #72).** It clocks
  the compile of a tiny fixture — a few milliseconds — so scheduler jitter
  on a shared CI runner routinely moved it past the ±10 % bound and failed
  the build, which was the intermittent red on `main`. The metric is still
  measured and shown in the report; it just never counts as a regression.
  The structural metrics (file / routine / pragma counts, generated bytes,
  LLM cost) are deterministic and remain gated.
